...
for(year in ac(2009:2015)){
  ...
  tacsat            <- tacsat[which(idxH>0 | idxO>0),]
  ...
  eflalo            <- subset(eflalo,LE_RECT %in% unique(coordsEflalo[which(idxH>0 | idxO > 0),"LE_RECT"]))
...
#-------------------------------------------------------------------------------
  ...
  tacsatEflalo$LENGTHCAT<- cut(tacsatEflalo$VE_LEN,breaks=c(0,12,15,200))
  tacsatEflalo$LENGTHCAT<- ac(tacsatEflalo$LENGTHCAT)
  tacsatEflalo$LENGTHCAT[which(tacsatEflalo$LENGTHCAT == "(0,12]")]   <- "<12"
